...
export function FilterMultiSelectPopup({
  ...
  const openPopup = (currentValues: string[]) => {
      ...
      window.open((globalThis as any).__toAppUrl(`/popup/multi-select?openerTabId=${tabId}&source=multi-select-${name}&options=${optionsParam}&selected=${selectedParam}&title=${titleParam}`),
        'MultiSelectPopup',
