      ...
      filters: [
        // 1??        { 
          ...
          onButtonClick: (value?: any, e?: React.MouseEvent<HTMLElement>) => {
            ...
            window.open((globalThis as any).__toAppUrl(`/popup/customer-search?customerNumber=${customerNumber}&openerTabId=${tabId}`), 'CustomerSearch', 'width=1600,height=800');
          }
        },
        ...
        { 
          ...
          onButtonClick: (value?: any, e?: React.MouseEvent<HTMLElement>) => {
            e?.preventDefault();
            const name = value || '';
            window.open((globalThis as any).__toAppUrl(`/popup/customer-search?customerName=${name}&openerTabId=${tabId}&sourceFilter=applicantName`), 'CustomerSearch', 'width=1600,height=800');
          }
        },
        ...
        // 12??        { 
          ...
          onButtonClick: (value: any) => {
            ...
            window.open((globalThis as any).__toAppUrl(`/popup/unprocessed-amount-processing${amountParam}`),
          ...
          actions={[
            { 
              ...
              onClick: () => {
                ...
                if (customerNumberToPass) {
                  window.open((globalThis as any).__toAppUrl(`/popup/document-search?customerNumber=${customerNumberToPass}&openerTabId=${tabId}`), 'DocumentSearch', 'width=1600,height=800');
                } else {
            ...
            { 
              ...
              onClick: () => {
                ...
                if (customerNumber) {
                  window.open((globalThis as any).__toAppUrl(`/popup/document-scan?customerNumber=${customerNumber}`), 'DocumentScan', 'width=1600,height=800');
                } else {
                  toast.warning("議고쉶??怨좉컼???놁뒿?덈떎.", {
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button 
                  ...
                  onClick={() => {
                    ...
                    if (accountNumber) {
                      window.open((globalThis as any).__toAppUrl(`/popup/modification-history?customerNumber=${customerNumber || ''}&accountNumber=${accountNumber}&openerTabId=${tabId}`),
                        'ModificationHistory',
                ...
                <Button 
                  ...
                  onClick={() => {
                    ...
                    if (accountNumber) {
                      window.open((globalThis as any).__toAppUrl(`/popup/transaction-history?accountNumber=${accountNumber}`), 'TransactionHistory', 'width=1600,height=800');
                    } else {
                      toast.warning("議고쉶??怨꾩쥖媛 ?놁뒿?덈떎.", {
                        description: "癒쇱? 議고쉶寃곌낵 ?뚯씠釉붿뿉???됱쓣 ?좏깮?섏뿬 二쇱떗?쒖삤.",
                        duration: 3000,
