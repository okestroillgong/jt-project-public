...
export default function LegalManagementPage() {
  ...
  const legalInquiryFilterLayout: FilterLayout = [
    {
      ...
      filters: [
        { 
          ...
          onButtonClick: (val, e) => {
            e?.preventDefault();
            window.open((globalThis as any).__toAppUrl(`/popup/customer-search?openerTabId=${tabId}`),
              "CustomerSearch",
        ...
        { 
          ...
          onButtonClick: (val, e) => {
             ...
             window.open((globalThis as any).__toAppUrl(`/popup/case-inquiry?caseNumber=${caseNumber}&openerTabId=${tabId}`),
               "CaseInquiry",
               "width=1600,height=800"
             );
          }
  ...
  const createDetailLayout = (type: string): FilterLayout => {
    const handleCustSearch = (val: any, e: any) => {
        e?.preventDefault();
        window.open((globalThis as any).__toAppUrl(`/popup/customer-search?openerTabId=${tabId}`), "CustomerSearch", "width=1600,height=800");
    };
    const handleCaseSearch = (val: any, e: any) => {
        e?.preventDefault();
        const caseNumber = val?.code || '';
        window.open((globalThis as any).__toAppUrl(`/popup/case-inquiry?caseNumber=${caseNumber}&openerTabId=${tabId}`), "CaseInquiry", "width=1600,height=800");
    };
    const handleDeptSearch = (val: any, e: any) => {
        e?.preventDefault();
        window.open((globalThis as any).__toAppUrl(`/popup/branch-management?openerTabId=${tabId}`), "BranchManagement", "width=1600,height=800");
    };
